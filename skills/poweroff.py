...
def poweroffProcess():
    sleep(time)
    try:
        notification.notify(title="Выключение устройства.", message="Программа Васисуалий завершает работу устройства.", timeout=10)
    except Exception:
        pass
    sleep(2)
    speak.speak("Выключаю компьютер. До свидания.", listWidget)
    os.system('shutdown -s')  # Выключение ПК
    exit()
...
def rebootProcess():
    sleep(time)
    try:
        notification.notify(title="Перезагрузка устройства.", message="Программа Васисуалий перезагружает устройство.", timeout=10)
    except Exception:
        pass
    sleep(2)
